...
import com.example.elera.databinding.HomeCategoryItemBinding
...
class CategoryRecyclerAdapter(val onCategoryChanged: OnCategoryChanged) : RecyclerView.Adapter<CategoryRecyclerAdapter.MyViewHolder>() {
    ...
    override fun getItemCount(): Int {
        return CategoryEnum.values().size
    }
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        if (position == 0) {
            ...
            val category = CategoryEnum.values()[position-1]
        ...
        holder.itemView.setOnClickListener {
            if (position != selectedPos){
                ...
                if (position == 0){
                    ...
                    onCategoryChanged.onCategoryChanged(CategoryEnum.values()[position-1])
                }
            }
    ...
    interface OnCategoryChanged{
        fun onCategoryChanged(category: CategoryEnum?)
    }
}